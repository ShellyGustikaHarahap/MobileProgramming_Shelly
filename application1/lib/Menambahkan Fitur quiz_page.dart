...
class _QuizPageState extends State<QuizPage> {
  ...
  void _nextQuestion(String selectedAnswer) {
    _selectedAnswers.add(selectedAnswer); // Simpan jawaban yang dipilih ke dalam list

    // Memeriksa jawaban benar atau salah
    if (selectedAnswer == _questions[_currentQuestionIndex].answer) {
      // Menampilkan Snackbar jika jawaban benar
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Jawaban Benar!', style: TextStyle(color: Colors.green)),
          duration: Duration(seconds: 1),
        ),
      );
    } else {
      // Menampilkan Snackbar jika jawaban salah
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Jawaban Salah!', style: TextStyle(color: Colors.red)),
          duration: Duration(seconds: 1),
        ),
      );
    }

    // Menaikkan indeks pertanyaan atau memindahkan ke halaman review
    Future.delayed(const Duration(seconds: 1), () {
      if (_currentQuestionIndex < _questions.length - 1) {
        ...
        // Jika semua pertanyaan sudah dijawab, tampilkan pesan
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Anda telah menjawab semua pertanyaan!'),
            duration: Duration(seconds: 2),
          ),
        );

        Future.delayed(const Duration(seconds: 2), () {
          // Pindah ke halaman review setelah 2 detik
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => QuizReviewPage(
                questions: _questions, // Kirim daftar pertanyaan ke halaman review
                selectedAnswers: _selectedAnswers, // Kirim jawaban yang dipilih ke halaman review
              ),
            ),
          );
        });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Kuis Biologi'), // Judul di AppBar
      ),
      body: Padding(
        padding: const EdgeInsets.all(16.0), // Memberikan padding di seluruh isi halaman
        child: Column(
